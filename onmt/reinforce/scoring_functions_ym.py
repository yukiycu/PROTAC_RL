...
from reward_ym.pre_lightgbm import lightgbm_ym
...
class YM():
    ...
    def __call__(self, smile):
        ...
        if mol:
            ...
            #isstandard = True
            if isstandard:
                #command = ". ~/.bash_profile && pyenv shell miniconda3-latest/envs/pre_permeability && python /home/murakami/workspace2024/PROTAC-RL/reward_ym/pre_lightgbm.py {mol}"                
                ...
                score = sigmoid(y_pred[0])
                score = lightgbm_ym(mol)
                #score = 0
                return score
            else:
                return 0.0
    ...
    def __call__(self, smiles):
        ...
        if self.worker == "YM":
            ...
            if mol:
                calc = Calculator(descriptors, ignore_3D=False)
                mordred = calc.pandas([mol])
                #pool.map(wrapper, [smiles, mordred])
                pool.map(self.worker, mordred)
        else:
...
def get_scoring_function(scoring_function, num_processes=None, **kwargs):
    ...
    scoring_function_classes = [PK, linker_length, CLOGP, MW, YM]
    ...
    print(scoring_functions)
    print(scoring_function_class)
